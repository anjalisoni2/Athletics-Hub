...
import { resolveEventImageUrl } from "../utils.js";
...
export async function renderEventDetails(container, eventId) {
  ...
  const allEvents = [...mockEvents, ...approvedEvents, ...localEvents];
    ...
    <div style="max-width:1200px;margin:0 auto;padding:24px">
      ...
      <div style="position:relative;margin-top:20px;margin-bottom:24px">
        <img src="${resolveEventImageUrl(event)}" 
             alt="${event.title}" 
             data-local="${event.localImageData || ''}"
             onerror="if(this.dataset.local && this.src !== this.dataset.local){this.src=this.dataset.local;}else{this.onerror=null;this.src='https://placehold.co/1200x400?text=Image+Not+Found'}"
             style="width:100%;height:320px;object-fit:cover;border-radius:16px;box-shadow:0 8px 30px rgba(0,0,0,0.12)" />
