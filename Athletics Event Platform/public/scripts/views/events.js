...
async function getApprovedEvents() {
 ...
 const mappedEvents = data.map((ev) => {
  ...
  return {
    ...
    // ✅ IMAGE FIX - Use full URL from backend
    imageUrl: ev.imageUrl
      ? ev.imageUrl
      : (ev.image ? (ev.image.startsWith('http') ? ev.image : `${getAPIBaseURL()}${ev.image}`) : "https://placehold.co/600x400?text=Event+Image"),
...
export async function renderEventsPage(container) {
  ...
  console.log(`📈 Total events after fetch: ${approvedEvents.length}`);
      ...
      <div class="container">
        <div style="display: grid; grid-template-columns: 1fr 1fr; gap: 3rem; align-items: center;">
          ...
          <div style="text-align: center;">
            <img src="${event.imageUrl}" alt="${event.title}" style="width: 100%; height: 300px; object-fit: cover; border-radius: 16px; box-shadow: 0 10px 30px rgba(0,0,0,0.3);">
          </div>
        </div>
      </div>
    </div>
